#!/usr/bin/env bun
import { existsSync, readFileSync, writeFileSync, mkdirSync, openSync, readSync, closeSync } from "fs";
...
// ─── TTY prompt ──────────────────────────────────
// Read directly from /dev/tty so it works even when
// stdin is a pipe (e.g. curl | bash -> exec bun setup.ts)
function prompt(question: string): string {
  ...
  try {
    ...
    // Fallback: no TTY available (CI, etc.) -> return empty
    return "";
  }
