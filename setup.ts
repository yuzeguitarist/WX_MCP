...
// ─── Step 1: Find backup ─────────────────────────
printHeader("wx-memory 微信记忆 MCP 配置向导");
print("");
...
const choice = prompt("选择 (1-6, 默认5): ") || "5";
...
const tools: ToolConfig[] = [
  ...
  },
  {
    ...
    configure() {
      ...
      writeFileSync(configPath, JSON.stringify(config, null, 2));
      print(`  ✅ Claude Code 配置已写入: ${configPath}`);
    },
  },
  {
    name: "Codex",
    ...
    configure() {
      const configPath = this.configPath;
      if (!existsSync(configPath)) {
        print(`  ⚠️  Codex 配置文件不存在: ${configPath}`);
        print(`  📋 请手动添加以下内容:`);
        printCodexToml();
        return;
      }
      let content = readFileSync(configPath, "utf-8");
      // Remove existing wx-memory config if any
      content = content.replace(/\[mcp_servers\.wx-memory\][\s\S]*?(?=\n\[|$)/, "");
      // Append new config
      const tomlBlock = buildCodexToml();
      content = content.trimEnd() + "\n\n" + tomlBlock + "\n";
      writeFileSync(configPath, content);
      print(`  ✅ Codex 配置已写入: ${configPath}`);
    },
  },
  {
    name: "Droid (Factory)",
...
print("");
print("测试命令:");
print(`  bun run ${ENTRY_POINT}`);
print("");
print("备份路径:");
print(`  ${finalBackupPath}`);
